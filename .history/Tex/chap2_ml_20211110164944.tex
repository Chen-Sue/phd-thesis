...
本章首先介绍了机器学习的基本概念（第\ref{sec:ml_intro}节）；接着简要介绍论文中涉及的机器学习模型的基础理论，包括LR（第\ref{sec:ml_lr}节）、KNN（第\ref{sec:ml_knn}节）、DT（第\ref{sec:ml_dt}节）、RF（第\ref{sec:ml_rf}节）、ETR（第\ref{sec:ml_etr}节）、GBRT（第\ref{sec:ml_gbr}节）、SVR（第\ref{sec:ml_svr}节）、CNN（第\ref{sec:ml_cnn}节）、LSTM-RNN（第\ref{sec:ml_lstm}节）；然后引出这些模型训练前的必要准备，包括利用滑动窗口法（第\ref{sec:ml_slide}节）、数据归一化（第\ref{sec:ml_scaler}节）、优化器（第\ref{sec:ml_optimizer}节）、优化目标函数（第\ref{sec:ml_loss}节）、性能度量（第\ref{sec:ml_performance}节）、欠拟合和过拟合（第\ref{sec:ml_fitting}节）、计算工具与平台（第\ref{sec:ml_computer}节）。
