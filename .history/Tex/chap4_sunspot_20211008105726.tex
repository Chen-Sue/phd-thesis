...
太阳黑子时间序列呈现出复杂的非线性、非高斯型分布的特征。第\ref{sec:ml_cnn}节和第\ref{sec:ml_lstm}节提到，LSTM-RNN和1DCNN均适用于时间序列数据，而且原始记录持续了上百年，因此这里选择了LSTM-RNN和1DCNN。考虑到LSTM-RNN和1DCNN对处理时间序列数据有着不同的优势，综合这两种网络的特点，得到LSTM-1DCNN。
