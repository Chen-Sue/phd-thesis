\chapter{机器学习对南加州地区的地震中期预报}\label{chap:机器学习对南加州地区的地震中期预报}
...
\mathrm{log}N=a-bM.
\end{equation}
其中$N>0$是指震级不小于阈震级$M_c$的地震事件的累积数量，$b$是斜率。公式~\eqref{eq:Gutenberg-Richter}的$b$一般可以通过两种方法获取，即最小二乘法（the least square method，简称LSM）和最大似然估计法（ the maximum likelihood estimation method，简称MLEM）。$b$不仅能够反映研究地区地下的地质构造，还能够反映地球物理特性。在获取$b$后，我们很容易得到其他几个地震因子（$a$，$\eta$，$\Delta M$）.
...
\subsection{地震因子}\label{sec:Database}
...
\renewcommand{\arraystretch}{1}
\resizebox{\textwidth}{!}{
%   \begin{tabular}{p{0.15cm}p{1.5cm}p{6.5cm}p{6cm}}
\begin{tabular}{llll}
  \toprule
  No & Attribute & Formula & Description \\
  \midrule
  1 & $M_{\mathrm{max}}$ & & Maximum magnitude within the time window. \\ 
  2 & $\mathrm{Frequency}$ & frequency & Earthquake frequency. \\ 
  3 & $M_{\mathrm{mean}}$ & $\displaystyle M_{\mathrm{mean}}=\frac{\sum_i{M_i}}{\mathrm{frequency}}$ & Mean magnitude within the time window. \\ 
  4 & $b_{\mathrm{lstsq}}$ & $\displaystyle b_{\mathrm{lstsq}}=\frac{n\sum_i{(M_i\mathrm{log} N_i)}-\sum_i{M_i}\sum_i\mathrm{log}N_i}{(\sum_i{M_i})^2-n\sum_i{{M_i}^2}}$ & Gutenberg-Richter law’s $b$-value calculated by the least square method. \\
  5 & $b_{\mathrm{mle}}$ & $\displaystyle b_{\mathrm{mle}}=\frac{\mathrm{log}e}{M_{\mathrm{mean}}-M_C}, M_C=3.0$ & Gutenberg-Richter law’s $b$-value calculated by the maximum likelihood estimation method. \\ 
  6 & $a_{\mathrm{lstsq}}$ & $\displaystyle a_{\mathrm{lstsq}}=\frac{\sum_i{(\mathrm{log}N_i+bM_i)}}{n}$ & Gutenberg-Richter law’s $a$-value calculated by the least square method. \\ 
  7 & $\Delta M$ & $\displaystyle \Delta M=M_{\mathrm{max, observed}}- \frac{a_{\mathrm{lstsq}}}{b_{\mathrm{lstsq}}}$ & Magnitude deficit. \\ 
  8 & $\eta$ & $\displaystyle \eta=\sqrt{\frac{\sum_i{(\mathrm{log} {N_i}}-(a_{\mathrm{lstsq}}-b_{\mathrm{lstsq}}{M_i}))^2}{n}}$ & Root mean square error calculated by the least square method. \\ 
  9 & $\sqrt{E}$ & $\displaystyle \sqrt{E}=\sum_i{\sqrt{E_i}}$, $\displaystyle E_i=10^{11.8+1.5\mathrm{M}_i}.$ & Square root of seismic energy \citep{Last2016predicting,asim2017earthquake}. 
  \\ 
  10 & $\mathrm{Lat}_{\mathrm{mean}}$ & $\displaystyle \mathrm{Lat}_{\mathrm{mean}}=\frac{\sum_i{\mathrm{Lat}_i}}{\mathrm{frequency}}$ & Mean latitude. \\ 
  11 & $\mathrm{RMSE}_{\mathrm{Lat}}$ & $\displaystyle \mathrm{RMSE}_{\mathrm{Lat}}=\sqrt{\frac{\sum_i{(\mathrm{Lat}_i-\mathrm{Lat}_\mathrm{mean})}^2}{\mathrm{frequency}}}$ & Root mean square deviation of latitude. \\ 
...
\center
    ...
    \bicaption{不同模型预测区块1未来一年最大震级的预测效果。(a)含64个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 1 in next year by different models. (a) LSTM-RNN with 64 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
...
\center
    ...
    \bicaption{不同模型预测区块2未来一年最大震级的预测效果。(a)含32个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 2 in next year by different models. (a) LSTM-RNN with 32 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
...
\center
    ...
    \bicaption{不同模型预测区块3未来一年最大震级的预测效果。(a)含32个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 3 in next year by different models. (a) LSTM-RNN with 32 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
...
\center
    ...
    \bicaption{不同模型预测区块4未来一年最大震级的预测效果。(a)128个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 4 in next year by different models. (a) LSTM-RNN with 128 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
...
\center
    ...
    \bicaption{不同模型预测区块5未来一年最大震级的预测效果。(a)32个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 5 in next year by different models. (a) LSTM-RNN with 32 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
...
\center
    ...
    \bicaption{不同模型预测区块6未来一年最大震级的预测效果。(a)32个LSTM神经元的LSTM-RNN，(b)SVR，(c)LR，(d)RF，(e)DT，(f)KN，(g)GBR，(h)ETR。(观测数据集：灰色；预测的训练集：蓝色；预测的验证集：紫色；预测的测试集：红色。)}{The effects of predicting the maximum magnitute of block 6 in next year by different models. (a) LSTM-RNN with 32 LSTM cells, (b)SVR, (c)LR, (d)RF, (e)DT, (f)KN, (g)GBR, (h)ETR. (observed dataset: grey, predicted train dataset: blue, predicted validation dataset: purple, predicted test dataset: red).}
