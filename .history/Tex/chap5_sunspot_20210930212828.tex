...
已有很多研究中采取不同的方法预测即将到来的第25太阳周的太阳黑子变化，得到结果并不统一，其原因可能是太阳黑子具备极其复杂的易变性。表\ref{tab:ss_number_different_studies}列出部分关于第25太阳周的研究。例如，\citet{covas2019neural}使用具备时空特性的神经网络，预测第25太阳周可能是有史以来太阳黑子数最低的时期，预测峰值出现的时间为2022年至2023年。与本研究结果类似，\citet{okoh2018hybrid}利用多维混合模型估计MSSN，得到第25太阳周的峰值为122.1（\pm 18.2），达到时间为2025年1月（\pm 6个月）。导致做出的结果差异，可能的原因还有数据集长度的差异，预处理的差异，以及选择模型的差异。
