...
% 针对LSTM-RNN和LSTM-1DCNN，我们采用了三层架构，LSTM-1DCNN采用了四层。采取层数较少的网络层数是因为数据集只有几百对样本，三到四层的网络足以较好地拟合数据集。因各种网络具有的独特性质，在设置以下超参数时会有所差异。针对LSTM-RNN，前两层含LSTM神经元，最后一层为全连接层。隐藏层的节点数设为32和16，输出层为全连接层。针对1DCNN，第一层/第二层为1维卷积层，过滤器个数、过滤器大小、步长分别为32/16、3、1。卷积层后连接了最大池化层，池化大小为2，步长为1。最后一层为全连接层。针对LSTM-1DCNN，第一层为1维卷积层，过滤器个数、过滤器大小、步长分别为128、3、1。卷积层后连接了最大池化层，池化大小为2，步长为1。第二、三层均含有LSTM神经元，神经元个数分别为64和32。最后一层为全连接层。 

\subsection{预测未来一个月太阳黑子强度}\label{sec:ss_result_one}

\begin{table}[!htbp]
    \centering
    \bicaption{不同模型、不同输入窗口预测未来一个月太阳黑子强度。}{Predicting sunspot smplitudes
        Solar minimum and maximum timings and smplitudes of sunspot numbers.}
    \label{tab:ss_max_min}
    \footnotesize
    \renewcommand{\arraystretch}{1}
    \begin{tabular}{cccccccc}
