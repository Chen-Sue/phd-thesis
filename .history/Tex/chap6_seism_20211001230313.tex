...
第\label{sec:ml_fitting}节详细介绍了过拟合产生的原因。模型的泛化误差来源于偏差、方差和噪音。假设输入地震特征因子向量为$\Vector{X}$，输出未来一年最大震级为$M_{\text{predicted}}$，实际发生震级为$M_{\text{observed}}$，要拟合的目标函数为$f(\Vector{X})$，训练的函数为$\hat{f}(\Vector{X})$，则偏差为：
\begin{equation}
  \label{eq:seism_bias}
...
\end{equation}
高方差意味着模型本身与期望的模型$f(\Vector{X})$差距大，因此会导致过拟合的问题。
