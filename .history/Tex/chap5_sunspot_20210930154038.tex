...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{1.0\textwidth}
      ...
      \end{subfigure}
    \vspace{-2cm}
    \bicaption{不同输入窗口的最佳模型预测未来一个月的泉流量。（a-b）输入窗口3个月，最佳模型RF；（c-d）输入窗口4个月，最佳模型RF；（e-f）输入窗口5个月，最佳模型RF；（h-g）输入窗口6个月，最佳模型RF。}{Predicting the next monthly spring discharge by the best models with different input windows. (a-b)Input windows:3, best model:RF; (c-d)Input windows:4, best model:RF; (e-f)Input windows:5, best model:RF; (g-h)Input windows:4, best model:RF.}
