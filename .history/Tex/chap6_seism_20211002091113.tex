...
鉴于第\ref{sec:seism_result_1}节的结果，这里我们试图延长输出窗口，即预测未来10年的最大震级。这里，我们同样采取了8种不同类型的模型。模型输入是整个区块在某个特定时刻前10年时间窗口内的16个预报因子，即输入层为$6\times 10\times 12$个节点，输出是整个区块下一年的最大震级。数据集划分比例为8：2。LSTM-RNN网络的设置可超参数的使用同上。
