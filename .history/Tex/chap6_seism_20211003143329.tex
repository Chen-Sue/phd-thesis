...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \bicaption{不同模型预测区块1未来一年最大震级的时间序列图（数据集划分比例为0.8:0.2）。}{The time series of predicting the maximum magnitute with the split ratio 0.8:0.2 of block 1 in next year by different models.}
...
\begin{table}[!htbp]
  \bicaption{不同模型预测整个区块未来一年最大震级的拟合指标效果。}{The metrics for predicting the maximum magnitute in next year by different models.}
