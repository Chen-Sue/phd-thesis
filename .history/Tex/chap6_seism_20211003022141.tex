...
鉴于第\ref{sec:seism_result_6}节的结果，这里试图不对区域进行划分滑动，采用滑动窗口法获取模型的输入和输出。同样采取了8种不同类型的模型。模型输入是整个区块在某个特定时刻前6年时间窗口内16个预报因子，即输入为$6\times 12\times 16=72$个，输出是整个区块下一年的最大震级。数据集划分比例为0.8：0.2。LSTM-RNN网络的设置和超参数的使用同上。
