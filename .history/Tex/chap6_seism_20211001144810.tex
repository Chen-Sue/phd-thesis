...
本章节结构安排如下。第\ref{sec:seis_data_method}节描述了使用的数据集可方法，具体包括地震目录属性描述、地震因子的计算、时空窗口法、数据的预处理过程以及使用的模型。第\ref{sec:seis_result}节描述地震预报的试验过程，并将试验结果可视化。第\ref{sec:seis_conclusion}节对本章节中地震预报进行了总结，并对下一步研究进行了展望。

\section{数据与方法}\label{sec:seis_data_method}

\subsection{研究区域}\label{sec:seis_area}

加利福尼亚州位于北美和太平洋板块之间，地壳运动频繁，是地震多发地带。本研究选择南加州地区作为研究区域，此区域地理范围在$[\SI{32}{N}-\SI{37}{N}]$和$[\SI{114}{W}-\SI{122}{W}]$。图\ref{fig:seis_california}绘制了南加州地区区域构造和地震目录中记录地震的震级和发生位置。

\begin{figure}[!htbp]
  \centering
  \vspace{-0.1cm} 
  \includegraphics[width=0.60\textwidth]{Img/chap6_seis/seism_carlifornia.pdf}
...
地震目录来自南加州地震数据中心（Southern California Earthquake Data Center，简称SCEDC）\footnote{数据来源：\href{https://service.scedc.caltech.edu/ftp/catalogs/}{https://service.scedc.caltech.edu/ftp/catalogs/}}。地震目录记录从1932年1月1日开始记录，2021年9月19日截至，在研究区域中一共有831906条数据。每条记录包含以下信息：
\begin{itemize}
