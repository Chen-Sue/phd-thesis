...
以上的算法均可用于预报地震，但实际能够应用在预报地震中的方法寥寥无几。为了实现该目标，我们采取了LSTM-RNN\citep{Wang2017Earthquake,Bhatia2018EARTHQUAKE,berhich2020lstm}以及其他机器学习算法（SVR、LR、RF、DT、KN、GBR、ETR），这些方法可以拟合较为复杂的任务。其中，LSTM-RNN是特别适用于分析时间序列任务，而我们的震级正是时间序列\cite{Morales2010Pattern,Werner2009High}。
