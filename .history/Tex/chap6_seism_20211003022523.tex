...
这里采取了8种不同类型的模型，分别为LSTM-RNN、SVR、LR、RF、GBR、DT、KNN、ETR。模型输入是6个区块在某个特定时刻前6年时间窗口内的16个地震因子，即输入为$6\times 6\times 12\times 16=6912$个，输出是某个区块下一年的最大震级。数据集划分比例为0.8：0.2。针对两层的LSTM-RNN，第一层含32个LSTM神经元，最后一层为全连接层。训练经过1000回合，网络每层使用ReLU激活函数。批量训练数据量设为128。Adam方法作为优化器。学习率初始值设为$10^{-3}$。同时训练次数每增加10次，学习率会减小$1-10^{-6}$倍。所有试验均使用“早停”方案。
...
鉴于第\ref{sec:seism_result_6}节的结果，这里试图不对区域进行划分滑动，采用滑动窗口法获取模型的输入和输出。同样采取了8种不同类型的模型。模型输入是整个区块在某个特定时刻前6年时间窗口内16个预报因子，即输入为$6\times 12\times 16=1152$个，输出是整个区块下一年的最大震级。数据集划分比例为0.8：0.2。LSTM-RNN网络的设置和超参数的使用同上。
...
鉴于第\ref{sec:seism_result_1}节的结果，这里试图延长输出窗口，即预测未来十年的最大震级。这里同样采取了8种不同类型的模型。模型输入是整个区块在某个特定时刻前10年时间窗口内的16个预报因子，即输入为$6\times 10\times 12\times 16=11520$个，输出是整个区块未来十年的最大震级。数据集划分比例为0.8：0.2。LSTM-RNN网络的设置和超参数的使用同上。

\begin{table}[!htbp]
  \bicaption{不同模型预报整个区块未来十年最大震级的拟合指标效果。}{The metrics for predicting the maximum magnitute in next 10 year by different models.}
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \bicaption{不同模型预报整个区块未来十年最大震级的预报效果（数据集划分比例为0.8：0.2）。}{The effects of predicting the maximum magnitute with the split ratio 8:2 in next 10 year by different models.}
