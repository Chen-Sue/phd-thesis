...
要进行机器学习，先要有数据集。数据集由很多数据对象组成，一个数据对象代表一个实体。数据对象又称样本、实例、数据点或对象。属性（Attribute）是一个数据字段，表示数据对象的一个特征。在研究太阳黑子变化过程中，只考虑了历史太阳黑子变化如何影响未来太阳黑子变化。因此输入和输出特征都来自同一属性。本论文从一种属性（太阳黑子强度）出发，利用机器学习探索时间序列数据。
...
\begin{figure}[!htbp]
  ...
  \vspace{-1.4cm}
  \bicaption[1949年至2021年月均太阳黑子数时间序列]{1949年至2021年月均太阳黑子数时间序列。}{Monthly mean total sunspot number time series between 1749 and 2021.}
  \label{fig:ss_number_series}
\end{figure}

本章采用两种时间序列数据集。第一类为月均太阳黑子数（Monthly Mean Total Sunspot Number，简称MSSN），来自SILSO（Sunspot Index and Long-Term Solar Observation）网站\footnote{数据来源：WDC-SILSO，Royal Observatory of Belgium，Brussels，\href{http://sidc.be/silso/datafiles}{http://sidc.be/silso/datafiles}}发布的太阳黑子2.0版本。MSSN数据集跨越了$\sim$273年（从1749年1月至2021年8月），包含3272条记录。截至目前，MSSN数据集包含了24个太阳周。表\ref{tab:ss_max_min}展示了这24个太阳周太阳黑子数量、峰值和谷值出现时间。第1太阳周为1755年2月至1766年5月，目前正处于第25太阳周的起始阶段\footnote{参考：\href{www://sidc.be/silso/cyclesmm}{www://sidc.be/silso/cyclesmm}}。图\ref{fig:ss_number_series}使用1749年1月至2021年8月期间月均太阳黑子数时间序列数据，绘制了太阳黑子数的变化趋势图。图\ref{fig:ss_number_series}显示，MSSN数据集的分布均呈现出右偏的。\citet{panigrahi2021forecasting}指出，MSSN数据集的峰度大于3。鉴于MSSN数据集呈现复杂的非高斯型分布，因此较难预测出太阳黑子数的变化趋势。
...
\begin{table}[!htbp]
  \centering
  \bicaption[测试阶段不同模型和输入时间窗口预测未来1个月太阳黑子面积的效果指标]{测试阶段不同模型和输入时间窗口预测未来1个月太阳黑子面积的效果指标。}{The indicators for predicting the next monthly sunspot areas by different models with different input windows for testing stage.}
  ...
  \begin{tabular}{ccccc}
    ...
    LSTM & 72-[64-32]-1 & \textbf{0.0021} & \textbf{0.0454} \\
     & 132-[64-32]-1 & 0.0023 & 0.0479 \\
     & 264-[64-32]-1 & 0.0024 & 0.0492 \\
    \hline
    ...
    LSTM-1DCNN & 72-[64-32]-1 & 0.0021 & 0.0454 \\
     & 132-[64-32]-1 & \textbf{0.0021} & \textbf{0.0453} \\
     & 264-[64-32]-1 & 0.0023 & 0.0479 \\
    \bottomrule
\end{tabular}
\end{table}
