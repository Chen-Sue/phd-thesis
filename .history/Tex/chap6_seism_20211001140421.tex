...
很多方法都被用来预报地震，这些方法可被分成由物理驱动的模型和由数据驱动的模型。由物理驱动的模型包括AMR（Accelerating Moment Release）方法\citep{Ben2002Accelerated}，RTL（the Region-Time-Length）算法\citep{Sobolev2007On}， ETAS（the Epidemic-Type Aftershock Sequence）\citep{Ogata1986Statistical}等。由数据驱动的方法，即机器学习，尤其是深度学习被越来越多地被应用到地震学中\citep{Alves2006Earthquake,Madahizadeh2009prediction,Panakkat2007Neural,Sunkara2009Model}。
