...
鉴于第\ref{sec:seism_result_1}节的结果，这里试图延长输出窗口，即预测未来十年的最大震级。这里同样采取了8种不同类型的模型。模型输入是整个区块在某个特定时刻前10年时间窗口内的16个预报因子，即输入为$16$个，输出是整个区块未来十年的最大震级。数据集划分比例为0.8：0.2。LSTM-RNN网络的设置和超参数的使用同上。
