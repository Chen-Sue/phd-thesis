...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{1.0\textwidth}
      ...
      \end{subfigure}
    \vspace{-2cm}
    \bicaption{最佳模型预测未来一个月的太阳黑子数。（a）LSTM-RNN；（b）1DCNN；（c）LSTM-1DCNN。}{Predicting the next monthly sunspot numbers by the best models. (a-b)Input windows:3, best model:RF; (c-d)Input windows:4, best model:RF; (e-f)Input windows:5, best model:RF; (g-h)Input windows:4, best model:RF.}
