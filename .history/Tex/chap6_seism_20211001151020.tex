...
$b$值一般可以通过两种方法获取，分别为最小二乘（Least Square，简称LS）法和最大似然估计（ the maximum likelihood estimation method，简称MLE）方法。$b$不仅能够反映研究地区地下的地质构造，还能够反映地球物理特性。在获取$b$后，我们很容易得到其他几个地震因子（$a$，$\eta$，$\Delta M$）.
