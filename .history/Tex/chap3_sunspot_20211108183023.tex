...
\begin{table}[!htbp]
  \centering
  \bicaption[不同输入时间窗口和层数下LSTM-1DCNN预测未来72个月太阳黑子数量的效果指标]{不同输入时间窗口和层数下LSTM-1DCNN预测未来72个月太阳黑子数量的效果指标。}{The indicators for predicting the next 72 monthly sunspot numbers by LSTM-1DCNN with different input windows and layers.}
