...
第\label{sec:ml_fitting}节详细介绍了过拟合产生的原因。过拟合是由偏差、方差和噪音导致的。假设输入地震特征因子向量为$\Vector{X}$，输出未来一年最大震级为$M_{\text{predicted}}$，实际发生震级为$M_{\text{observed}}$，要拟合的目标函数为$f(\Vector{X})$，训练的函数为$\hat{f}(\Vector{X})$，则偏差为：
\begin{equation}
  \label{eq:seism_bias}
