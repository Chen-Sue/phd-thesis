...
在第\label{sec:ml_prepare}节，我们已经介绍了窗口滑动法。在探索太阳黑子和泉流量时采取了窗口滑动法，从而将原始观测数据集转化为监督学习数据。与前面的窗口滑动法不同的是，这里我们不仅在时间上对数据集进行滑动划分，还要在空间上对数据集进行滑动划分，这样做的目的是在更小区域范围内预报未来最大震级。因为，除了预报最大震级外，我们还要关注该最大震级发生的空间位置，而且空间范围越小越好。
