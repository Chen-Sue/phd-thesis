...
时间窗口按月滑动，空间窗口按照表\ref{tab:seism_windows_30}中区块序号进行滑动，即从研究区域的左上角开始向右移动一格（纬度\SI{4}{\degree}），到达区域边缘后，再向下滑动，滑动间隔为（经度\SI{1.5}{\degree}）。向下滑动开始时沿着左边缘会向右移\SI{0.5}{\degree}再开始滑动，滑动时会向右移动一格（\SI{3.5}{\degree}）。第三行第一块沿着左边缘会向右移\SI{1}{\degree}再开始滑动，滑动时会向右移动一格（\SI{3}{\degree}）。这样重复划分几次后，一共得到了9个空间区块。这样划分区块的目的是，左下角区域是海域，地震目录中缺失了这部分的数据。
