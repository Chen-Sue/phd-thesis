...
要进行机器学习，先要有数据集。数据集由很多样本组成。在研究太阳黑子活动时，只考虑了历史太阳黑子活动如何影响未来太阳黑子活动。因此模型的输入和输出都来自同一种特征。本论文从一种特征（太阳黑子活动）出发，利用机器学习探索时间序列数据。
...
\begin{table}[!htbp]
  \centering
  \bicaption[最佳模型预测的太阳黑子活动]{最佳模型预测的太阳黑子活动。}{Prediction the sunspot amplitudes by the best models.}
  \label{tab:ss_out_1}
  \footnotesize
...
\begin{table}[!htbp]
\centering
\bicaption[最佳模型预测未来72个月太阳黑子活动的最大值]{最佳模型预测未来72个月太阳黑子活动的最大值。}{Prediction the sunspot maximum amplitudes by the best models.}
\label{tab:ss_out_72}
\footnotesize
\begin{tabular}{cccccccc}
    \toprule
