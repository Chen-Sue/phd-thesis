...
从表\ref{tab:seism_block1}至表\ref{tab:seism_block6}中展示了6个不同区块在不同模型下预报未来一年最大震级的拟合指标效果。由表\ref{tab:seism_block1}至表\ref{tab:seism_block6}可知，两层的LSTM-RNN

LSTM-RNN的表现来看，两层的LSTM-RNN在隐藏层节点数为32、64、128均出现了很大程度的过拟合现象。可见LSTM-RNN网络相对于本数据集而言，过于复杂，因此这里我们使用其他几种机器学习方法，仍然发现了很大程度的过拟合现象。从这些结果可以看出，机器学习对南加州地区的地震中期预报是失败的。
