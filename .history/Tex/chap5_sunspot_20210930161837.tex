...
图\ref{fig:ssn_out_1}绘制了输入窗口为72的最佳模型预测未来一个月的太阳黑子数量，图\ref{fig:ssn_out_1}绘制了输入窗口为132的最佳模型预测未来一个月的太阳黑子数量。从图\ref{fig:spr_out_1}可以看出，对于预测值和测试集，RF具备良好的拟合能力。除了异常值，所有的结果均显示，绝大部分的预测值和观测值的绝对误差基本在$\SI{0.1}{m^{3}/s}$以内。利用这些最佳的RF预测未来一个月（即2019年1月的龙子祠泉流量），可以得到较为可靠的结果。
