...
当然本方法在太阳黑子预测上也存在局限性。从上面讨论可以看出，输入窗口和输出窗口直接决定了模型的构建，而且输出窗口对预测结果有非常大的影响，这使得输入和输出序列长度成为好模型构建的一项前提条件。在前面分析过程中，输出长度越长，结果越不可靠。如果要提供非常准确的预报，可以将预报时长设置为1年，每隔一段时间更新模型，重新预测下一年的太阳黑子变化，这样也能为未来一年做准备。
