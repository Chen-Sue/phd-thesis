...
\begin{longtable}{cclcccccccc}
  ...
  \bicaption[不同模型预测区块2至区块6未来1年最大震级的拟合指标效果（数据集划分比例为0.8：0.2）（续）]{不同模型预测区块2至区块6未来1年最大震级的拟合指标效果（数据集划分比例为0.8：0.2）（续）。}{The metrics for predicting the maximum magnitute from block 2 to block 6 with the split ratio 0.8:0.2 in next year by different models (continued).} \\
  \toprule
  \multirow{2}*{区块序号} & \multirow{2}*{模型} & \multicolumn{2}{c}{训练集} & \multicolumn{2}{c}{测试集} \\
  \cmidrule(lr){3-4} \cmidrule(lr){5-6} \noalign{\smallskip}
  & & MSE & RMSE & MSE & RMSE \\
