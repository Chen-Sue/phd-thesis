...
这里，我们采取了8种不同类型的模型，分别为LSTM-RNN、SVR、LR、RF、GBR、DT、KNN、ETR。模型输入是6个区块在某个特定时刻前6年时间窗口内的16个预报因子，即输入层为$6\times 16=96$个节点，输出是某个区块下一年的最大震级。针对LSTM-RNN，前两层含LSTM神经元，最后一层为全连接层。隐藏层的节点数设为32和16，输出层为全连接层。
