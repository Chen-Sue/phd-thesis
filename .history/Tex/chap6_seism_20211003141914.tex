...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \bicaption{不同模型预报整个区块未来十年最大震级的预报效果（数据集划分比例为）。}{The effects of predicting the maximum magnitute with the split ratio 0.90:0.10 in next ten years by different models.}
  \bicaption{不同模型预测整个区块未来十年最大震级的时间序列图（数据集划分比例为0.90：0.10）。}{The time series of predicting the maximum magnitute with the split ratio 0.90:0.10 in next ten years by different models.}
