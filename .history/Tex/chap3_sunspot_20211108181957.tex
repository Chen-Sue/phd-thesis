...
\begin{table}[!htbp]
  \centering
  \bicaption[测试阶段不同模型和输入时间窗口预测未来1个月太阳黑子数量效果指标]{测试阶段不同模型、不同输入时间窗口预测未来1个月太阳黑子数量效果指标。}{The indicators for predicting the next monthly sunspot numbers by different models with different input windows for testing stage.}
