...
本节围绕预处理方法和机器学习模型展开。这里使用不同种类的神经网络模拟太阳黑子的动态变化特征。为了更精准地捕获太阳黑子变化趋势，本章使用了近几百年来观测到的太阳黑子数量/面积，试图从历史观测数据中提取关键信息，预测未来太阳活动。太阳黑子强度可以从长期历史记录中预测，依赖历史的时间长度从1个月到几个太阳周不等。将历史观测数据集转化为监督学习时间序列数据集。太阳黑子变化趋势可以被近似表达为：
\begin{equation}
...
这里简要介绍不同模型的超参数配置。所有模型的输入节点数$k$由输入窗口决定，输出节点数$f$由输出窗口决定。以下是本章所有神经网络通用的超参数。训练经过1000回合，网络每层均会使用ReLU激活函数。批量训练数据量设为132。Adam方法作为优化器。学习率初始值设为$10^{-3}$。同时训练次数每增加10次，学习率会减小$1-10^{-6}$倍。所有试验均使用“早停”方案。
...
\begin{table}[!htbp]
  \centering
  \bicaption{测试阶段不同模型、不同输入窗口预测未来1个月太阳黑子数量效果指标。}{The indicators for predicting the next monthly sunspot numbers by different models with different input windows for testing stage.}
...
\begin{table}[!htbp]
  \centering
  \bicaption{测试阶段不同模型、不同输入窗口预测未来1个月太阳黑子面积的效果指标。}{The indicators for predicting the next monthly sunspot areas by different models with different input windows for testing stage.}
...
\begin{tabular}{cccccccc}
    ...
    \midrule
    数量 & 264 & 151.5516 & 2023年9月 & 174.71 & 2025年1月 & \textbf{132.86} & \textbf{2024年12月} \\
    面积 & 132/264/72 & 1016.323 & 2024年8月 & \textbf{1469.01} & \textbf{2025年3月} & 1397.7671 & 2024年4月 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  \centering
  \bicaption{测试阶段不同输入窗口和层数的LSTM-1DCNN预测未来72个月太阳黑子面积的效果指标。}{The indicators for predicting the next 72 monthly sunspot areas by LSTM-1DCNN with different input windows and layers for testing stage.}
...
本研究使用了LSTM-RNN、1DCNN以及LSTM-1DCNN预测未来太阳黑子变化，采用了MSSN和MSSA数据集，发现第25太阳周的最大太阳黑子数为132.8636（发生在2024年12月），最大太阳黑子面积为1469.0072（发生在2025年3月）。预测结果与\citet{covas2019neural}的研究结果不仅在黑子强度上类似，而且在发生时间上也相近。

同时研究还发现，模型性能会随着输出窗口的增加而下降，随着层数和节点数的增加而上升。如果要提供非常准确的预报，可以将输出时长设置为1年，每隔一段时间更新模型，重新预测下一年的太阳黑子变化。这也是本方法在太阳黑子预测上的局限性。输入窗口和输出窗口直接决定了模型的构建，而且输出窗口对预测结果有非常大的影响，这使得合适的输出序列长度成为良好模型构建的一项前提条件。
