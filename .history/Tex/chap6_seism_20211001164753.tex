...
\begin{equation}
  ...
  \end{split}
\end{equation}
其中，$t$代表当前时刻，$-\Delta$代表历史时间窗口（即输入时间窗口），$+T$代表未来时间窗口（即输出时间窗口），本研究中的采样间隔为1个月。$m=n$，即假设输入的降雨量和泉流量滞后时间相等。$Q(t,t+1,\ldots,t+T)$代表预测未来$T$个月的泉流量。$P_e(t-i\Delta t)(i\in\{1,2,\ldots,n\},e\in\{1,2,\ldots,9\})$代表历史第$e$区域第$i$个月的降雨量，$Q(t-j\Delta t)(j\in\{1,2,\ldots,m\})$代表历史第$j$个月的泉流量。
