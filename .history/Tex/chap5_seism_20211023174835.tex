...
以上均是基于回归的基础上构建机器学习模型，且绝大多数模型拟合的结果均出现了过拟合现象。这里我们尝试将预测未来10年的最大震级看作是分类问题，从不同的角度去观测数据集变化对结果的影响程度。\citet{nan2019xinjiang}指出，地震震级通常可允许的误差为\pm 0.3，故本文将震级测定看成是分类问题，对震级大小进行等级分类，具体分类情况如表 1所示，将ML2.0~2.2分为第1类，ML2.3~2.5为第2类，…，ML＞7.6为第20类，共将ML＞2.0的震级分成20个不同等级类别.
