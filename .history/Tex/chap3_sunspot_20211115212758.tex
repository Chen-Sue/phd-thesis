...
\begin{table}[!htbp]
  \centering
  \bicaption[不同模型在不同的输入时间窗口长度下预测未来1个月太阳黑子数的拟合指标效果]{不同模型和输入时间窗口长度下预测未来1个月太阳黑子数的拟合指标效果}{The indicators for predicting the next monthly sunspot numbers by different models with different input windows.}
