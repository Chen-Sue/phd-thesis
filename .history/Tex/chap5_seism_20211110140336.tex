...
以上均是基于回归的基础上构建机器学习模型，且大多数模型拟合的结果均出现了过拟合问题。这里我们尝试将预测未来10年的最大震级看作是分类问题，从不同的角度去观测数据集变化对结果的影响程度。\citet{nan2019xinjiang}指出，地震震级通常可允许的误差为$\pm$ 0.3。本节对震级大小进行等级分类，具体分类情况如表\ref{tab:seism_class}所示，将[3.0,3.2]分为第1类，[3.3,3.5]为第2类，$\ldots$，大于等于7.5为第16类，共将标签分成16个不同等级类别。  
