...
要进行机器学习，先要有数据集。数据集由很多数据对象组成，一个数据对象代表一个实体。数据对象又称样本、实例、数据点或对象。属性（Attribute）是一个数据字段，表示数据对象的一个特征。在太阳黑子研究太阳黑子变化过程中，只考虑了历史太阳黑子变化如何影响未来太阳黑子变化。因此输入和输出特征都来自同一属性。本论文从一种属性（太阳黑子强度）出发，利用机器学习探索时间序列数据。
