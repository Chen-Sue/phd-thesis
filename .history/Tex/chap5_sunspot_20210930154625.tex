...
\begin{subfigure}[b]{1.0\textwidth}
    ...
    \end{subfigure}
\vspace{-2cm}
\bicaption{最佳模型预测未来一个月的太阳黑子数。（a）LSTM-RNN；（b）1DCNN；（c）LSTM-1DCNN。}{Predicting the next monthly sunspot numbers by the best models. (a) LSTM-RNN; (b) 1DCNN; (c) LSTM-1DCNN.}
\label{fig:spr_discharge}
\end{figure}

\begin{table}[!htbp]
\centering
\bicaption{最佳模型预测2021年9月的太阳黑子强度。}{Prediction the sunspot amplitudes in September 2021.}
\label{tab:spr_one}
\footnotesize
\begin{tabular}{ccccc}
    \toprule
    输入时间窗口 & 3 & 4 & 5 & 6 \\
    \midrule
    2019年1月的泉流量（$\SI{}{m^{3}/s}$） & 2.9852 & 2.9945 & 3.0106 & 3.0515 \\
    \bottomrule
\end{tabular}
\end{table}
